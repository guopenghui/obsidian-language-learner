<template>
    <NConfigProvider :theme="theme" :theme-overrides="themeOverrides">
        <NModal v-model:show="props.show" :mask-closable="false"
                preset="dialog"
                :title="t('Learning New Words')"
                style="width: 620px; max-height:800px; background-color: #363636; overflow: scroll;"
        >
            <div id="langr-learn-panel">
                ...
                <NForm :model="model" label-placement="top" label-width="auto" :rules="rules"
                    ...
                    <NFormItem :label="t('Expression')" :label-style="labelStyle" path="expression">
                        <NInput size="small" v-model:value="model.expression"
                    ...
                    <NDynamicInput v-model:value="model.sentences" :create-button-props="{ size: 'small' }"
                        ...
                        <template #="{ index, value }">
                            <div style="display: flex;
                                ...
                                padding: 3px;
                            ">
                                <NFormItem :show-label="false" :path="`sentences[${index}].text`"
                                           :rule="sourceRule">
                ...
                <slot name="action">
                    ...
                    <div style="margin-top: 10px;">
                        <NButton size="small" style="--n-width: 100%;" attr-type="submit" @click="submit"
                                 :loading="submitLoading">{{ t("Submit") }}
                        </NButton>
                    </div>
                </slot>
                <!-- 提交按钮 -->
                <!--                <div style="position: relative; bottom: 20px;">-->
                <!--                                            <NButton size="small" style="&#45;&#45;n-width: 100%; width: 100%;" attr-type="submit" @click="submit"-->
                <!--                             :loading="submitLoading">{{ t("Submit") }}-->
                <!--                    </NButton>-->
                <!--                </div>-->
                <!-- </NThemeEditor> -->
...
import {computed, CSSProperties, getCurrentInstance, PropType, ref, watch} from "vue";
...
async function submit() {
    ...
    // 删除之前
    console.log(props.word.expression)
    if (props.word.expression) {
        await plugin.db.DB().removeExpression(props.word.expression)
    }
...
#langr-learn-panel {
    ...
    //deep(. n-card-header), deep(. n-base-icon) {
    //    color: #fff !important;
    //}
